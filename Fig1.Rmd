...
all_rcps <- all_rcps %>%
  ...
  rename(adaptation = E1_OA0, no_adaptation = E0_OA0) %>%
  gather(Symbiont_Evolution, coral, 3:4)
...
geom_line(aes(color = Symbiont_Evolution)) +
facet_wrap(~ rcp) +   # facet by RCP
theme_bw() +          # set theme
xlim(c(2000,2100)) +  # set x limits
labs(x = "Year", y = "Mean % Coral Cover (n=1,925 reef cells)") 
